...
// Komponen header kolom yang bisa di-sort
function SortableTh({ label, sortKey, currentSort, currentDir, onSort, className = "" }) {
  const active = currentSort === sortKey;
  return (
    <th
      onClick={() => onSort(sortKey)}
      className={`cursor-pointer select-none hover:bg-primary-200 transition ${className}`}
    >
...
export default function ProdukListPage() {
  ...
  const handleEdit = (id_produk) => navigate(`/admin/produk-dan-stok/edit/${id_produk}`);
  const handleDetail = (id_produk) => navigate(`/admin/produk-dan-stok/detail/${id_produk}`);

  const handleDelete = async (id) => {
    if (!confirm("Yakin mau hapus produk?")) return;
    try {
      await fetch(`http://localhost:5000/api/produk/${id}`, { method: "DELETE" });
  ...
  return (
    <AdminLayout>
      <div className="produk-list-page">
        ...
        <div className="w-full h-[2px] bg-primary-200 mb-3" />
            ...
            <table className="produk-table w-full border-collapse">
              <thead className="bg-primary-100 sticky -top-1 z-10 border-b-2 border-[#D9D9D9]">
                <tr>
                  ...
                  <SortableTh label="Nama Produk" sortKey="nama"      currentSort={sortKey} currentDir={sortDir} onSort={handleSort} />
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan="5" className="p-6 text-center text-gray-400">Tidak ada produk</td></tr>
                ) : (
